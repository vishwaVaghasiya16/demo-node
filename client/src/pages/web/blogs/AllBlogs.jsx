import { Button, Container } from "react-bootstrap";
import { useDispatch, useSelector } from "react-redux";
import { useNavigate } from "react-router-dom";
import { CLIENT } from "../../../routes/routesConstants";
import { getMomentDate } from "../../../components/common/MomentFun";
import { blogPaginationThunk, imageBlogPagination } from "../../../store/actions";

const AllBlogs = () => {
  const { imageBlog, pagination } = useSelector((store) => store.Blogs);
  const nav = useNavigate();
  const dispatch = useDispatch();

  const handlePagination = async () => {
    await dispatch(
      imageBlogPagination({ page: pagination?.page + 1, limit: 15 })
    );
  };

  const data = Array.isArray(imageBlog)
    ? imageBlog.slice(9, imageBlog?.length)
    : [];
  if (data?.length > 0) {
    return (
      <section className="paddingBottom">
        <Container>
          <div className="d-grid grid-cols-2 gap-3 gap-lg-5">
            {data?.map((item, index) => {
              const thumbnail = item?.url;
              const title = item?.title;
              const description = item?.description;
              const date = item?.createdAt;
              const comment = item?.commentCount;
              return (
                <div
                  key={index}
                  className="d-md-grid grid-cols-2 gap-3 gap-lg-4"
                >
                  <div
                    className="aspect-1-0_8 bg-color-primary br-10 overflow-hidden zoom-hover cursor-pointer"
                    onClick={() => nav(`${CLIENT.BLOG_DETAILS}/${title}`)}
                  >
                    <img
                      src={thumbnail}
                      alt=""
                      className="h-100 w-100 object-fit-cover zoom transition-transform"
                    />
                  </div>
                  <div>
                    <div className="d-none d-sm-flex flex-wrap align-items-center mt-3">
                      <p className="d-flex align-items-center text-color-secondary p-0 m-0 fs-14 me-3">
                        <i className="ri-calendar-2-line fs-24 me-2"></i>
                        {getMomentDate(date, "MMMM DD, YYYY")}
                      </p>
                      {/* <p className="d-flex align-items-center text-color-secondary p-0 m-0 fs-14">
                        <i className="ri-message-2-line fs-24 me-2"></i>
                        {comment} COMMENTS
                      </p> */}
                    </div>
                    <h2 className="fs-20 fs-md-5 text-color-primary mt-2">
                      {title}
                    </h2>
                    <p className="fs-14 text-color-secondary truncate-line-3">
                      {description}
                    </p>
                    <Button
                      className={`primary-btn fs-14 mt-3 mt-sm-4`}
                      onClick={() => nav(`${CLIENT.BLOG_DETAILS}/${title}`)}
                    >
                      Read More
                    </Button>
                  </div>
                </div>
              );
            })}
          </div>
          {pagination?.page < pagination?.totalPages && (
            <Button
              className="primary-btn fs-14 d-block mx-auto mt-20"
              onClick={handlePagination}
            >
              Load More
            </Button>
          )}
        </Container>
      </section>
    );
  }
};

export default AllBlogs;
